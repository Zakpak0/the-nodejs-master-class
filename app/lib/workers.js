...
workers.validateCheckData = function(originalCheckData){
    ...
    originalCheckData.url.length > 0 ?
    ...
    originalCheckData.timeoutSeconds >= 1 ? originalCheckData.timeoutSeconds : false

    //Set the keys that may not be set (if the workers have neveer seen this check before)
    ...
    //If all the checks pass, pass the data along tot he next step in the process
...
workers.performCheck = function(originalCheckData){
    //Prepare the initial check outcome from
    let checkOutcome = {
        'error' : false,
        'responseCode' : false,
    }
    ...
    //Parse the hostname and the path out of th eoriginal check
    ...
    let req = _moduleToUse.request(requestDetials,function(res){
...
//Process the check outcome, update the check data as neeeded, trigger an alert if needed
...
workers.processCheckOutcome = function(originalCheckData,checkOutcome){
    ...
    _data.update('checks',newCheckData.id,newCheckData,function(err){
        if(!err){
            //Send the new check data tot he next phase int he process if needed
            if(alertWarrented){
