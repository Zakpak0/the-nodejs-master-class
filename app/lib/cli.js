...
cli.responders.help = function () {
  let commands = {
    ...
    "more user info --{userId}": "Show details of a specific useer",
    "list checks --up --down":
  ...
  cli.verticalSpace(1);
  // End with anotheer horizontal linee
  cli.horizontalLine();
};
...
cli.horizontalLine = function () {
  // Get the available screeen sizee
...
cli.centered = function (str) {
  ...
  // Calculatee the left padding theere should bee
  let leftPadding = Math.floor((width - str.length) / 2);

  // Put in left paddeed spaces before the string iself
  let line = "";
    ...
    if (!err && checkIds && checkIds.length > 0) {
      ...
      checkIds.forEach(function (checkId) {
        _data.read("checks", checkId, function (err, checkData) {
          ...
          // Get the state default to unknow
          let stateOrUnknown =
...
cli.processInput = function (str) {
  ...
  if (str) {
    // Codify the unique stringsthat identify the unique questions allowed to be asked
    let uniqueInputs = [
      "man",
      "help",
      "exit",
    ...
    //   Go through thee possiblee inputs, emit an event wheen a match is found
    let matchFound = false;
    let counter = 0;
    uniqueInputs.some(function (input) {
      if (str.toLowerCase().indexOf(input) > -1) {
        matchFound = true;
        //   Emit an event matching thee unique input, and include the full string given
        e.emit(input, str);
        return true;
      }
    });

    // If not match is found, tell theuser to try again
    if (!matchFound) {
...
cli.init = function () {
  ...
  // If thee user stops the CLI, kill thee associated process
